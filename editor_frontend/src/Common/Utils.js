...
const Utils = {
  ...
  GroupByKey(objArray, keyName){
    var result = {};  

    objArray.map((obj)=>{
      var stringKeyName = Utils.md5(obj[keyName]);
      
      if(!result.hasOwnProperty(stringKeyName)){
        result[stringKeyName] = [obj];
      }else{
        result[stringKeyName].push(obj);
      }
    });
