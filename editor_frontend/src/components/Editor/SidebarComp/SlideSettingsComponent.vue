    ...
    <template v-if="toggleOpen">
      ...
      <template v-if="slide != null">
        ...
        <div class="row">
          ...
          <button class="cta-button" @click="TakeScreenshot">
            Take Screenshot
          </button>
        </div>
        <div class="row">
          <VectorField
            title="Kamera Position"
            ref="camPos"
            :object="slide"
            path="slides.CameraPosition"
            ><button class="icon-button" @click="GetCamPos"><SpotIcon /></button
          ></VectorField>
        </div>
        <div class="row">
          <VectorField
            ...
            ref="targetPos"
            :object="slide"
            path="slides.CameraTarget"
            ><button class="icon-button" @click="GetTargetPos">
              <SpotIcon /></button
          ></VectorField>
        </div>
        <div class="row">
...
// Zeigt alle verfügbaren Eigenschaften eines Slides an
// Es kann eine Vorschau für das Slide generiert werden und die Kamera Positionen festgelegt werden
export default {
  components: {
    TextField,
    VectorField,
  ...
  methods: {
    TakeScreenshot() {
      this.$store.state.xr.Renderer.instance.render(
        this.$store.state.xr.Scene,
        this.$store.state.xr.Camera.instance
      );

      var strMime = "image/jpeg";
      var imgData = this.$store.state.xr.Renderer.instance.domElement.toDataURL(
        strMime
      );
      ...
      this.urltoFile(
        imgData,
        "screenshot_slide_" + this.slide.id + ".jpg",
        "image/jpeg"
      )
        .then((file) => {
          console.log(file);
        ...
        .then((formData) => {
          this.PostData(config.CMS_BASE_URL + "/upload/", formData).then(
            (response) => {
              if (this.slide.PreviewImage != null) {
                this.Delete(
                  config.CMS_BASE_URL +
                    "/upload/files/" +
                    this.slide.PreviewImage.id
                ).then((res) => {
                  this.Put(
                    config.CMS_BASE_URL + "/slides/" + this.slide.id,
